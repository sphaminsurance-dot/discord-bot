// gatewayBot.js — Node 20 (Railway)
    ...
    // -------------------- Active-marker cache --------------------
    // Each entry: { client_key, active_marker_channel_id, active_marker_message_id }
    let activeMarkers = [];

    async function fetchActiveMarkers() {
      try {
        const resp = await dashboardRequest("GET", "/api/bot/active-markers");
        if (resp.statusCode === 200 && resp.data && Array.isArray(resp.data.markers)) {
          activeMarkers = resp.data.markers;
    ...
    async function markerReaction({ discord_user_id, message_id, emoji, action }) {
      const resp = await dashboardRequest("POST", "/api/bot/marker-reaction", {
        discord_user_id,
        message_id,
        emoji,
        action,
      ...
      console.log(`API_MARKER_REACTION_ERROR: status=${resp.statusCode}`, resp.data);
      return null;
    }
    ...
    async function handleMoneybagReaction(reaction, user, added) {
      try {
        ...
        // Use the dedicated marker-reaction endpoint which resolves the client by message_id
        const action = added ? "add" : "remove";
        const result = await markerReaction({
        ...
        if (result?.ok) {
          console.log(`MARKER_REACTION_OK: user=${user.id} action=${action} client=${result.client_key} status=${result.status || "unchanged"}`);
        } else {
          console.log(`MARKER_REACTION_FAIL: user=${user.id} action=${action}`);
        }
      } catch (err) {
